...
public:
  ...
  void addFramePose(double x, double y, double z, double r_x, double r_y,
                    double r_z, double r_w);
  void addKeyframePose(double kfTimestamp, double x, double y, double z,
                       double r_x, double r_y, double r_z, double r_w);

  void addRANSACStats(const std::string &type, int iterations, int inliers,
                      double time, bool success, bool refined);

  std::vector<VBEELine> vbeeLines;
  void addVBEELine(int mpID, float p_e, float model_est, float observability,
                   bool seen);
