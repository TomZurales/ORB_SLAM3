      ...
      if (mCurrentFrame.mvpMapPoints[i]) {
        if (!mCurrentFrame.mvbOutlier[i]) {
          ...
          if (!mbOnlyTracking) {
            ...
            if (pMP && !pMP->isBad()) {
              ...
              if (global_vbee_settings.fake_eliminations && pMP->fakeBad &&
                  !pMP->countedForStats) {
                global_tracked_stats.AddBadElimination();
                pMP->countedForStats = true;
              }
            }
          }
        ...
        if (pMP && !pMP->isBad()) {
          if (global_vbee_settings.in_use) {
            ...
            if (vbeeScore < global_vbee_settings.bad_threshold) {
...
bool Tracking::Relocalization() {
  ...
  if (!bMatch) {
    ...
    double relocalization_time =
        std::chrono::duration_cast<std::chrono::duration<double>>(
            std::chrono::steady_clock::now() - t1)
            .count();
    global_tracked_stats.AddRelocalizationTime(relocalization_time);
    return true;
  }
}
