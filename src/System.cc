...
Sophus::SE3f System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight,
  ...
  if (vbeeSettings.in_use) {
    std::set<ORB_SLAM3::MapPoint *> missedPoints;
    std::set<ORB_SLAM3::MapPoint *> seenPoints;

    // Determine the set of map points that "should be" visible from this
    // location
    for (const auto &mp : mpAtlas->GetCurrentMap()->GetAllMapPoints()) {
      if (!mp)
        continue;
      if (mp->isBad())
        continue;
      if (!mpTracker->mCurrentFrame.isInFrustumSimple(mp)) {
        mp->isInCameraView = false;
        continue;
      }

      mp->isInCameraView = true;
      if (std::find(mTrackedMapPoints.begin(), mTrackedMapPoints.end(), mp) ==
          mTrackedMapPoints.end()) {
        missedPoints.insert(mp);
      } else {
        seenPoints.insert(mp);
      }
    }

    for (auto mp : missedPoints) {
      Observation obs = Observation{
          .v = (mp->GetWorldPos() - mpTracker->mCurrentFrame.GetCameraCenter()),
          .s = 0.0};
      if(mp->vbee.Update(obs) < 0.05)
      {
        mp->SetBadFlag();
      }
    }
    for (auto mp : seenPoints) {
      Observation obs = Observation{
          .v = (mp->GetWorldPos() - mpTracker->mCurrentFrame.GetCameraCenter()),
          .s = 1.0};
      mp->vbee.Update(obs);
