...
namespace ORB_SLAM3
{
        ...
        mbDeactivateLocalizationMode(false), mbShutDown(false)
  {
    ...
    ppe = new PointProbabilityEngine(PPEModel::ICOSAHEDRON);
                                   ...
                                   string filename)
  {
    ...
    ppe->setCamera(mpTracker->mpCamera);
    ppe->setMap(mpAtlas->GetCurrentMap());
    ...
    // Convert MapPoint* to PPEPointInterface*
    vector<PPEPointInterface *> ppePoints;
    for (MapPoint *pMP : mpTracker->mCurrentFrame.mvpMapPoints)
    {
      if (pMP && !pMP->isBad())
      {
        ppePoints.push_back(static_cast<PPEPointInterface *>(pMP));
      }
    }
    ppe->Update(Tcw.matrix().inverse(), ppePoints);
