...
bool System::RelocalizeFrame(const cv::Mat &imLeft, const cv::Mat &imRight,
  ...
  return mpTracker->RelocalizeFrame(imLeftToFeed, imRightToFeed, timestamp,
                                    filename);
}
...
Sophus::SE3f System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight,
  ...
  for (size_t i = 0; i < seenMapPoints.size(); i++) {
    ...
    if (bestObservations.find(seenMapPoints[i]) != bestObservations.end()) {
      // If the map point was previously not seen, replace it with the current
      // seen operation
      if (bestObservations[seenMapPoints[i]].s == 0.0) {
        Observation o =
            Observation{.v = (mpTracker->mCurrentFrame.GetCameraCenter() -
                              seenMapPoints[i]->GetWorldPos())
                                 .normalized(),
                        .s = 1.0};
  ...
  std::vector<MapPoint *> allMapPoints =
      mpTracker->mpAtlas->GetCurrentMap()->GetAllMapPoints();
  // for (auto kf : localKeyFrames)
  // {
  //     if (!kf || kf->isBad())
  //         continue;
  //     auto kfMapPoints = kf->GetMapPoints();
  //     localMapPoints.insert(localMapPoints.end(), kfMapPoints.begin(),
  //     kfMapPoints.end());
  // }
  ...
  for (auto mp : allMapPoints) {
    ...
    if (mpTracker->mCurrentFrame.isInFrustumSimple(mp, 0.1, 10)) {
      Observation o = Observation{
          .v = (mpTracker->mCurrentFrame.GetCameraCenter() - mp->GetWorldPos())
                   .normalized(),
          .s = 0.0};
  ...
  if (vbee_loop_count % 10 == 0) {
    for (auto observation : bestObservations) {
      ...
      if (new_p_e < 0.1) {
        ...
        std::cout << "Removing Map Point " << observation.first->mnId
                  << std::endl;
      }
    }
...
void System::SaveKeyFrameTrajectoryEuRoC(const string &filename) {
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO ||
      ...
      Eigen::Quaternionf q = Twb.unit_quaternion();
      Eigen::Vector3f twb = Twb.translation();
      DatabaseManager::Instance().addKeyframePose(
          pKF->mTimeStamp, twb(0), twb(1), twb(2), q.x(), q.y(), q.z(), q.w());
    } else {
      Sophus::SE3f Twc = pKF->GetPoseInverse();
      Eigen::Quaternionf q = Twc.unit_quaternion();
      Eigen::Vector3f t = Twc.translation();
      DatabaseManager::Instance().addKeyframePose(
          pKF->mTimeStamp, t(0), t(1), t(2), q.x(), q.y(), q.z(), q.w());
    }
  }
  f.close();
}
...
void System::GlobalBundleAdjustment() {
  ...
  bool stopped = false;
  Optimizer::GlobalBundleAdjustemnt(mpAtlas->GetCurrentMap(), 50, &stopped, 0,
                                    true);
}
