...
DatabaseManager &DatabaseManager::Instance() {
  if (!_instance)
    throw std::runtime_error("DatabaseManager not initialized. Call "
                             "DatabaseManager::Init(path) first.");
  return *_instance;
}
...
DatabaseManager::DatabaseManager(const std::string &test_name) {
  ...
  try {
    _common_db =
        std::make_unique<SQLite::Database>("common.db", SQLite::OPEN_READWRITE);
  ...
  _test_db = std::make_unique<SQLite::Database>(
...
void DatabaseManager::workerLoop() {
  while (true) {
    Task task;
    {
      ...
      queue_cv.wait(lock,
                    [this] { return stop_worker || !task_queue.empty(); });
...
void DatabaseManager::_clearTestData() {
  _test_db->exec("DELETE FROM VBEE;DELETE FROM FramePoses;DELETE FROM "
                 "KeyframePoses;DELETE FROM RANSACStats");
}
...
void DatabaseManager::_interactivePopulateTestDB() {
  ...
  // Prompt as to whether to use VBEE for this test. If not, end. Otherwise
  // prompt for all the parameters in the Params table.
  std::string line;
  ...
  bool use_vbee = true;
  if (!line.empty() &&
      (line == "n" || line == "N" || line == "no" || line == "NO")) {
    use_vbee = false;
    std::cout << "VBEE disabled for this test. A Params row will still be "
                 "added with defaults and use_vbee=0.\n";
  }
  ...
  if (!line2.empty() &&
      (line2 == "n" || line2 == "N" || line2 == "no" || line2 == "NO")) {
    weight_ransac = false;
  ...
  try {
    SQLite::Statement insert(
        *_test_db, "INSERT INTO Params (name, use_vbee, weight_ransac, k, n, "
                   "a_th, f_th, init_p_e, damp_coeff, init_obs, "
                   "obs_damp_coeff) VALUES (?,?,?,?,?,?,?,?,?,?,?);");
    insert.bind(1, name);
...
std::vector<Trajectory> DatabaseManager::getAllTrajectories() {
  ...
  SQLite::Statement query(*_common_db,
                          "SELECT * FROM Trajectories ORDER BY Id ASC;");
...
void DatabaseManager::addTrackTime(double time) {
  enqueue([this, time]() {
    SQLite::Statement query(
        *_test_db, "INSERT INTO TrackTimes (time, traj) VALUES (?, ?);");
    query.bind(1, time);
    query.bind(2, traj);
    query.exec();
  });
...
void DatabaseManager::addRelocTime(double time) {
  enqueue([this, time]() {
    SQLite::Statement query(
        *_test_db, "INSERT INTO RelocTimes (time, traj) VALUES (?, ?);");
    query.bind(1, time);
    query.bind(2, traj);
    query.exec();
  });
...
void DatabaseManager::addFramePose(double x, double y, double z, double r_x,
                                   double r_y, double r_z, double r_w) {
  enqueue([=, this]() {
    SQLite::Statement query(
        *_test_db, "INSERT INTO FramePoses (timestamp, traj, x, y, z, r_x, "
                   "r_y, r_z, r_w) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);");
...
void DatabaseManager::addKeyframePose(double kfTimestamp, double x, double y,
                                      double z, double r_x, double r_y,
                                      double r_z, double r_w) {
  enqueue([=, this]() {
    // Use a small epsilon to find the closest timestamp
    const double epsilon = 1e-5;
    SQLite::Statement stmt(*_test_db,
                           "SELECT traj FROM FramePoses WHERE ABS(timestamp - "
    ...
    if (stmt.executeStep()) {
      ...
      std::cerr << "No matching FramePose found for timestamp " << kfTimestamp
                << std::endl;
      return;
    }

    SQLite::Statement query(
        *_test_db, "INSERT INTO KeyframePoses (timestamp, traj, x, y, z, r_x, "
                   "r_y, r_z, r_w) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);");
...
void DatabaseManager::writeVBEELines() {
  enqueue([this]() {
    ...
    SQLite::Statement query(*_test_db,
                            "INSERT INTO VBEE (mpID, timestamp, traj, p_e, "
                            "est, obs, seen) VALUES (?, ?, ?, ?, ?, ?, ?);");
...
void DatabaseManager::addVBEELine(int mpID, float p_e, float model_est,
                                  float observability, bool seen) {
  enqueue([=, this]() {
...
void DatabaseManager::addRANSACStats(const std::string &type, int iterations,
                                     int inliers, double time, bool success,
                                     bool refined) {
  enqueue([=, this]() {
    SQLite::Statement query(
        *_test_db,
        "INSERT INTO RANSACStats (timestamp, type, iterations, inliers, time, "
        "success, refined) VALUES (?, ?, ?, ?, ?, ?, ?);");
    query.bind(1, timestamp);
    query.bind(2, type);
...
Trajectory DatabaseManager::getTrajectoryById(int id) {
  ...
  SQLite::Statement query(
      *_common_db, "SELECT Id, Name, Path FROM Trajectories WHERE Id = ?;");
  query.bind(1, id);
...
std::vector<int> DatabaseManager::getTrajectoryIDs() const {
  ...
  std::vector<int> ids;
  SQLite::Statement query(*_test_db,
                          "SELECT Id FROM Trajectories ORDER BY [Order] ASC;");
  while (query.executeStep()) {
...
DBParams DatabaseManager::getParams() const {
  ...
  DBParams p{};
  SQLite::Statement query(
      *_test_db, "SELECT use_vbee, weight_ransac, k, n, a_th, f_th, init_p_e, "
                 "damp_coeff, init_obs, obs_damp_coeff FROM Params LIMIT 1;");
  if (query.executeStep()) {
