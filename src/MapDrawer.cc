...
std::vector<float> getHeatMapColor(float value) {
  value = std::max(0.0f, std::min(1.0f, value)); // Clamp value between 0 and 1
...
void MapDrawer::DrawMapPoints() {
  ...
  for (size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
    ...
    // #ifdef TCZ_THESIS
    auto probExists =
        mpPointProbability->getPointProbabilityMetadata(vpMPs[i]).getPexists();
...
>>>>>>> origin/pre-8_20
    ...
    std::vector<float> getHeatMapColor(float value)
    {
        ...
        // Red (0,0,0) -> Yellow (1,1,0) -> Green (0,1,0)
        if (value <= 0.5f)
        {
            // Red to Yellow: r=1, g=2*value, b=0
            r = 1.0f;
            g = 2.0f * value;
            b = 0.0f;
        }
        ...
        {
            // Yellow to Green: r=1-2*(value-0.5), g=1, b=0
            r = 1.0f - 2.0f * (value - 0.5f);
            g = 1.0f;
            b = 0.0f;
        }
