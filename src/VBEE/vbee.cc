...
VBEE::VBEE(int mpID) : mpID(mpID) {
  ...
  weight_ransac = dbParams.use_vbee;
  ...
  model = ObservabilityModel(
      ObservabilityModelParams{.k = dbParams.k,
                               .n = dbParams.n,
...
float VBEE::Update(Observation observation) {
  ...
  DatabaseManager::Instance().addVBEELine(
      mpID, std::min(0.999f, std::max(0.001f, p_e)), model_estimate,
      observability, seenStatus == SEEN);
