...
void ObservabilityModel::Update(const Observation &observation, float feedback)
{
    ...
    std::sort(prev_observations.begin(), prev_observations.end(),
              [&observation](const Observation &a, const Observation &b) {
                  return (a.v - observation.v).norm() < (b.v - observation.v).norm();
              });
    ...
    // Replace the nearest observation if no nearby opposite observations are found
