            ...
            if (mbVO) {
              for (int i = 0; i < mCurrentFrame.numKeyPoints; i++) {
                if (mCurrentFrame.mvpMapPoints[i] &&
                  ...
                  // mCurrentFrame.mvpMapPoints[i]->vbee.Update(mCurrentFrame.GetCameraCenter(),
                  // mCurrentFrame.mvpMapPoints[i]->GetWorldPos(), true);
                }
              }
            }
          } else if (bOKReloc) {
...
bool Tracking::TrackReferenceKeyFrame() {
  ...
  for (int i = 0; i < mCurrentFrame.numKeyPoints; i++) {
    ...
    //   // We now have our list of expected map points based on the optimized
    //   camera location.
    //   // We can go through our expected keypoints, and report any which are
    //   not in the current frame as
    //   // having a reduced probability of existing.
    //   auto expectedKeypoints =
    //   pPointProbability->GetExpectedMapPoints(&mCurrentFrame); for (auto
    //   mapPoint : expectedKeypoints)
    //   {
    //     if (std::find(mCurrentFrame.mvpMapPoints.begin(),
    //     mCurrentFrame.mvpMapPoints.end(), mapPoint) ==
...
bool Tracking::TrackLocalMap() {
  ...
  for (int i = 0; i < mCurrentFrame.numKeyPoints; i++) {
    if (mCurrentFrame.mvpMapPoints[i]) {
      if (!mCurrentFrame.mvbOutlier[i]) {
        ...
        mp->IncreaseFound();
        // mp->vbee.Update(mCurrentFrame.GetCameraCenter(), mp->GetWorldPos(),
        // true);
        if (!mbOnlyTracking) {
  ...
  // // We now have our list of expected map points based on the optimized
  // camera location.
  // // We can go through our expected keypoints, and report any which are not
  // in the current frame as
  // // having a reduced probability of existing.
  // auto expectedKeypoints =
  // pPointProbability->GetExpectedMapPoints(&mCurrentFrame); for (auto mapPoint
  // : expectedKeypoints)
  // {
  //   if (std::find(mCurrentFrame.mvpMapPoints.begin(),
  //   mCurrentFrame.mvpMapPoints.end(), mapPoint) ==
...
bool Tracking::RelocalizeFrame(const cv::Mat &imRectLeft,
  ...
  if (result) {
    ...
    if (pCurrentMap) {
      ...
      // Set last relocalization id (Relocalization() already sets
      // mnLastRelocFrameId, but keep it explicit if needed)
      mnLastRelocFrameId = mCurrentFrame.mnId;

      // Copy current frame into last frame so motion model and other code have
      // a valid last frame
      mLastFrame = Frame(mCurrentFrame);
      ...
      // Populate the local map structures used by
      // TrackLocalMap/SearchLocalPoints These functions rely on
      // mCurrentFrame.mvpMapPoints being already filled by Relocalization()
      UpdateLocalKeyFrames();
      UpdateLocalPoints();
