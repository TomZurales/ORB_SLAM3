...
bool Tracking::ParseCamParamFile(cv::FileStorage &fSettings) {
  ...
  if (sCameraName == "PinHole") {
    ...
    float fx, fy, cx, cy = 0.0f;
    float k1, k2, k3, k4 = 0.0f;
    mImageScale = 1.f;
      ...
      if (mState == LOST) {
        ...
        if (!mbVO) {
          ...
          if (bOKMM && !bOKReloc) {
            ...
            if (mbVO) {
              for (int i = 0; i < mCurrentFrame.numKeyPoints; i++) {
...
void Tracking::StereoInitialization() {
  if (mCurrentFrame.numKeyPoints > 500) {
    ...
    if (!mpCamera2) {
      ...
      for (int i = 0; i < mCurrentFrame.numKeyPointsLeft; i++) {
        ...
        if (rightIndex != -1) {
          ...
          pNewMP->AddObservation(pKFini,
                                 rightIndex + mCurrentFrame.numKeyPointsLeft);
          ...
          mCurrentFrame.mvpMapPoints[i] = pNewMP;
          mCurrentFrame
              .mvpMapPoints[rightIndex + mCurrentFrame.numKeyPointsLeft] =
              pNewMP;
        }
      }
    }
...
void Tracking::CheckReplacedInLastFrame() {
  for (int i = 0; i < mLastFrame.numKeyPoints; i++) {
  ...
  for (int i = 0; i < mCurrentFrame.numKeyPoints; i++) {
    ...
    if (mCurrentFrame.mvpMapPoints[i]) {
      if (mCurrentFrame.mvbOutlier[i]) {
        ...
        mCurrentFrame.mvpMapPoints[i] = static_cast<MapPoint *>(NULL);
        mCurrentFrame.mvbOutlier[i] = false;
        if (i < mCurrentFrame.numKeyPointsLeft) {
    ...
    // We now have our list of expected map points based on the optimized camera
    // location. We can go through our expected keypoints, and report any which
    // are not in the current frame as having a reduced probability of existing.
    pPointProbability->GetExpectedMapPoints(&mCurrentFrame);
  }
...
void Tracking::UpdateLastFrame() {
  ...
  const int Nfeat = mLastFrame.numKeyPointsLeft == -1
                        ? mLastFrame.numKeyPoints
                        : mLastFrame.numKeyPointsLeft;
  ...
  for (size_t j = 0; j < vDepthIdx.size(); j++) {
    ...
    if (bCreateNew) {
      ...
      if (mLastFrame.numKeyPointsLeft == -1) {
...
bool Tracking::NeedNewKeyFrame() {
  ...
  if (mSensor != System::MONOCULAR && mSensor != System::IMU_MONOCULAR) {
    int N = (mCurrentFrame.numKeyPointsLeft == -1)
                ? mCurrentFrame.numKeyPoints
                : mCurrentFrame.numKeyPointsLeft;
    for (int i = 0; i < N; i++) {
...
void Tracking::CreateNewKeyFrame() {
      ...
      mSensor != System::IMU_MONOCULAR) // TODO check if incluide imu_stereo
  {
    ...
    int N = (mCurrentFrame.numKeyPointsLeft != -1)
                ? mCurrentFrame.numKeyPointsLeft
                : mCurrentFrame.numKeyPoints;
    vDepthIdx.reserve(mCurrentFrame.numKeyPoints);
    for (int i = 0; i < N; i++) {
      float z = mCurrentFrame.mvDepth[i];
      if (z > 0) {
        vDepthIdx.push_back(make_pair(z, i));
    ...
    if (!vDepthIdx.empty()) {
      ...
      for (size_t j = 0; j < vDepthIdx.size(); j++) {
        ...
        if (bCreateNew) {
          ...
          if (mCurrentFrame.numKeyPointsLeft == -1) {
            mCurrentFrame.UnprojectStereo(i, x3D);
          } else {
          ...
          if (mCurrentFrame.numKeyPointsLeft != -1 &&
            ...
            mCurrentFrame.mvpMapPoints[mCurrentFrame.numKeyPointsLeft +
                                       mCurrentFrame.mvLeftToRightMatch[i]] =
                pNewMP;
            pNewMP->AddObservation(pKF,
                                   mCurrentFrame.numKeyPointsLeft +
                                       mCurrentFrame.mvLeftToRightMatch[i]);
            pKF->AddMapPoint(pNewMP, mCurrentFrame.numKeyPointsLeft +
                                         mCurrentFrame.mvLeftToRightMatch[i]);
          }
  ...
  while (nCandidates > 0 && !bMatch) {
    for (int i = 0; i < nKFs; i++) {
      ...
      if (bTcw) {
        ...
        for (int io = 0; io < mCurrentFrame.numKeyPoints; io++)
        ...
        if (nGood < 50) {
          ...
          if (nadditional + nGood >= 50) {
            ...
            if (nGood > 30 && nGood < 50) {
              sFound.clear();
              for (int ip = 0; ip < mCurrentFrame.numKeyPoints; ip++)
...
void Tracking::SetPointProbability(PointProbability *pPointProbability) {
  this->pPointProbability = pPointProbability;
}
=======
    Tracking::Tracking(System *pSys, ORBVocabulary *pVoc, FrameDrawer *pFrameDrawer,
                       MapDrawer *pMapDrawer, Atlas *pAtlas,
                        ...
                        if (bOKMM && !bOKReloc)
                        {
                            ...
                            if (mbVO)
                            {
                                for (int i = 0; i < mCurrentFrame.numKeyPoints; i++)
                                {
                                    ...
                                    {
                                        mCurrentFrame.mvpMapPoints[i]->IncreaseFound();
                                        // mCurrentFrame.mvpMapPoints[i]->vbee.Update(mCurrentFrame.GetCameraCenter(), mCurrentFrame.mvpMapPoints[i]->GetWorldPos(), true);
                                    }
                                }
                            }
                        }
            ...
            else
            {
                for (int i = 0; i < mCurrentFrame.numKeyPointsLeft; i++)
                {
                    ...
                    if (rightIndex != -1)
                    {
                        ...
                        pNewMP->AddObservation(pKFini, i);
                        pNewMP->AddObservation(pKFini, rightIndex + mCurrentFrame.numKeyPointsLeft);

                        pKFini->AddMapPoint(pNewMP, i);
                        pKFini->AddMapPoint(pNewMP, rightIndex + mCurrentFrame.numKeyPointsLeft);
                        ...
                        mCurrentFrame.mvpMapPoints[i] = pNewMP;
                        mCurrentFrame.mvpMapPoints[rightIndex + mCurrentFrame.numKeyPointsLeft] = pNewMP;
                    }
                }
            }
    ...
    void Tracking::UpdateLastFrame()
    {
        ...
        // We sort points according to their measured depth by the stereo/RGB-D sensor
        vector<pair<float, int>> vDepthIdx;
        const int Nfeat = mLastFrame.numKeyPointsLeft == -1 ? mLastFrame.numKeyPoints : mLastFrame.numKeyPointsLeft;
        vDepthIdx.reserve(Nfeat);
        for (int i = 0; i < Nfeat; i++)
        {
            float z = mLastFrame.mvDepth[i];
        ...
        for (int i = 0; i < mCurrentFrame.numKeyPoints; i++)
        {
            if (mCurrentFrame.mvpMapPoints[i])
            {
                if (!mCurrentFrame.mvbOutlier[i])
                {
                    ...
                    mp->IncreaseFound();
                    // mp->vbee.Update(mCurrentFrame.GetCameraCenter(), mp->GetWorldPos(), true);
                    if (!mbOnlyTracking)
                    {
                        if (mCurrentFrame.mvpMapPoints[i]->Observations() > 0)
                            mnMatchesInliers++;
    ...
    bool Tracking::NeedNewKeyFrame()
    {
        ...
        if (mSensor != System::MONOCULAR && mSensor != System::IMU_MONOCULAR)
        {
            int N = (mCurrentFrame.numKeyPointsLeft == -1) ? mCurrentFrame.numKeyPoints : mCurrentFrame.numKeyPointsLeft;
            for (int i = 0; i < N; i++)
            {
                if (mCurrentFrame.mvDepth[i] > 0 && mCurrentFrame.mvDepth[i] < mThDepth)
                {
    ...
    void Tracking::CreateNewKeyFrame()
    {
            ...
            mSensor != System::IMU_MONOCULAR) // TODO check if incluide imu_stereo
        {
            ...
            vector<pair<float, int>> vDepthIdx;
            int N = (mCurrentFrame.numKeyPointsLeft != -1) ? mCurrentFrame.numKeyPointsLeft : mCurrentFrame.numKeyPoints;
            vDepthIdx.reserve(mCurrentFrame.numKeyPoints);
            for (int i = 0; i < N; i++)
            {
                float z = mCurrentFrame.mvDepth[i];
                    ...
                    if (bCreateNew)
                    {
                            ...
                            mCurrentFrame.mvLeftToRightMatch[i] >= 0)
                        {
                            ...
                            pNewMP->AddObservation(
                                pKF, mCurrentFrame.numKeyPointsLeft + mCurrentFrame.mvLeftToRightMatch[i]);
                            pKF->AddMapPoint(pNewMP, mCurrentFrame.numKeyPointsLeft +
                                                         mCurrentFrame.mvLeftToRightMatch[i]);
                        }
    ...
    bool Tracking::RelocalizeFrame(const cv::Mat &imRectLeft,
                                   const cv::Mat &imRectRight,
                                   const double &timestamp,
                                   string filename)
    {
        ...
        bool result = Relocalization();
        if(result)
        {
            // Prepare tracking state so subsequent Track()/TrackStereo calls work
            Map *pCurrentMap = mpAtlas->GetCurrentMap();
            if(pCurrentMap)
            {
                // Lock the same mutex Track() uses when working with the map
                unique_lock<mutex> lock(pCurrentMap->mMutexMapUpdate);

                // Ensure reference KF is consistent
                if(!mCurrentFrame.mpReferenceKF)
                    mCurrentFrame.mpReferenceKF = mpReferenceKF;

                // Set last relocalization id (Relocalization() already sets mnLastRelocFrameId,
                // but keep it explicit if needed)
                mnLastRelocFrameId = mCurrentFrame.mnId;

                // Copy current frame into last frame so motion model and other code have a valid last frame
                mLastFrame = Frame(mCurrentFrame);

                // Make sure tracking reference keyframe pointers are consistent
                if(mCurrentFrame.mpReferenceKF)
                    mpReferenceKF = mCurrentFrame.mpReferenceKF;

                // Populate the local map structures used by TrackLocalMap/SearchLocalPoints
                // These functions rely on mCurrentFrame.mvpMapPoints being already filled by Relocalization()
                UpdateLocalKeyFrames();
                UpdateLocalPoints();

                // For visualization and other subsystems
                mpAtlas->SetReferenceMapPoints(mvpLocalMapPoints);
                if(mpMapDrawer)
                    mpMapDrawer->SetCurrentCameraPose(mCurrentFrame.GetPose());
                if(mpFrameDrawer)
    ...
    vector<MapPoint *> Tracking::GetLocalMapMPS() { return mvpLocalMapPoints; }
    vector<KeyFrame *> Tracking::GetLocalKeyFrames() { return mvpLocalKeyFrames; }
