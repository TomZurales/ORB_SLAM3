...
MapPoint::MapPoint(const Eigen::Vector3f &Pos, Map *pMap, Frame *pFrame,
  ...
  const int level =
      (pFrame->numKeyPointsLeft == -1)    ? pFrame->mvKeysUn[idxF].octave
...
#ifdef TCZ_THESIS
float MapPoint::getProbExists() { return this->probExists; }

void MapPoint::offsetProbExists(float offset) {
  this->probExists += offset;
  float newProbExists =
      std::max<float>(0.0, std::min<float>(1.0, this->probExists));
