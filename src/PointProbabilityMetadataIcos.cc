...
namespace ORB_SLAM3 {
PointProbabilityMetadataIcos::PointProbabilityMetadataIcos(MapPoint *pMapPoint)
    : pMapPoint(pMapPoint) {}
float PointProbabilityMetadataIcos::getPexists() { return pExists; }
...
void PointProbabilityMetadataIcos::setIcosDist(int face, float dist) {
  if (icosDists[face] < dist) {
    std::cout << "Increasing distance on face " << face << " of map point "
              << pMapPoint->mnId << " to " << dist << std::endl;
    icosDists[face] = dist;
  }
}
...
int PointProbabilityMetadataIcos::getClosestIcosFaceIndex(
    Eigen::Vector3f cameraPose) {
