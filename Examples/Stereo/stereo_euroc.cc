...
int main(int argc, char **argv) {
  ...
  for (seq = 0; seq < num_seq; seq++) {
    ...
    for (int ni = 0; ni < nImages[seq]; ni++, proccIm++) {
      ...
      if (std::find(split_stamps.begin(), split_stamps.end(), tframe) != split_stamps.end()) {
        std::cout << "SPLIT STAMP FOUND. SETTING ALL MAP POINTS TO BE FROM PREVIOUS MAP" << std::endl;
        std::vector<ORB_SLAM3::Map *> vpMaps = SLAM.mpAtlas->GetAllMaps();
        for (ORB_SLAM3::Map *pMap : vpMaps) {
